...
def can_place(grid, piece, x, y):
    ...
    for py in range(DIM):
        gy = y + py
        if x > OFFSET and ((1 << (x - OFFSET)) - 1) & piece[py] > 0:
            # check that we don't over shift
            #
            # x=4 mask is ...# (bit will get truncated)
            # x=5 mask is ..## (bits will get truncated)
            # x=6 mask is .###
            # x=7 mask is ####
            #
            # row: ####
...
def intpiece(piece: list[str]) -> list[int]:
    """
    Transform a literal shape definition into its packed int representation.
    """
    vals = []
...
def two(jets):
    ...
    # Find the cycle and then capture the minimum information
    # to compiute the result
...
def find_cycle(jets):
    """
    Finds a cycle in the height growth pattern.

    A necessary condition for a cycle to occur is that the current piece
